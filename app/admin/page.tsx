...
export default async function AdminPage() {
  ...
  const [
    ...
    supabase.from("products").select("id,slug,brand,name,size,sku,barcode,image_url,description,inci,inci_verified,public_price,vat_rate,units_per_box,weight_kg,low_stock_threshold,stock,active,family,category,cost,archived").order("brand").order("name"),
    ...
    supabase.from("warehouses").select("id,name,sort").order("sort"),
    supabase.from("inventory_levels").select("product_id,warehouse_id,on_hand"),
  ]);
