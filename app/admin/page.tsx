...
export default async function AdminPage() {
  ...
  const [{ data: products }, { data: orders }, { data: requests }, { data: clients }] = await Promise.all([
    ...
    supabase.from("profiles").select("id,full_name,company,cif,phone,role,tariff_code,status,commercial_agreement,gc_mandate_status,created_at").neq("role", "admin").order("created_at", { ascending: false }),
  ]);
  ...
  return (
    <section className="page"><div className="wrap">
      ...
      <AdminPanel products={(products ?? []) as any} orders={(orders ?? []) as any} requests={(requests ?? []) as any} clients={(clients ?? []) as any} />
    </div></section>
  );
}
